                                    ...
                                    candidates_list)
{
        ...
        for (i in 1:length(candidates_list)){
                # Bleach data reduced.
                bleach_reduced <- bleach_data[, c("bin.score","DHW_5km_max", DHW.column)]
                ...
                # Calculating the model fit with eigenvectors included.
                model.fit <- glm(formula2,
                                 data = bleach_reduced,
                                 family = binomial)
...
create_plot_object <- function(column.names, plot.title, legend.title, x.label = " ", pred_table){
        p <- ggplot(data = pred_table, aes_string(x = column.names[1], y = "response", group = column.names[2])) + 
                ...
                labs(x = x.label, y = "Probability of severe bleaching", title = plot.title, col = legend.title, fill = legend.title) +
