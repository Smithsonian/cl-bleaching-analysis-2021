#################################################################
##                2020 Analysis - DHW * region                 ##
##                   By: Migdonio A. González                  ##
##                   Edited by: Sean Connolly                  ##
#################################################################
### Top 3 candidates.
candidate.number <- 201 # 7 EV
# candidate.number <- 221 # 8 EV
# candidate.number <- 161 # 8 EV
...
# Reduced dataframe.
bleach.reduced.2020 <- bleach_2020[,c("bin.score",dhw.column,"region")]
...
pr.2020 <- generate_predictions(m.2020, newdata.2020$no.ev, dhw.column)
pr.2020$region <- factor(pr.2020$region, levels = c("North","Central","South"))

pr.S.2020 <- generate_predictions(mS.2020, newdata.2020$ev, dhw.column)
pr.S.2020$region <- factor(pr.S.2020$region, levels = c("North","Central","South"))

# Creating plot objects.
plot.2020 <- create_plot_object("2020", dhw.column, "DHW (°C-week)", " ", pr.2020)
plot.S.2020 <- create_plot_object("2020", dhw.column, "DHW (°C-week)", " ", pr.S.2020)

# Plotting.
grid.arrange(plot.2020, plot.S.2020, nrow = 2)

# MORE NUMBERS
more.new <- data.frame(DHW_5km_max = c(8,8,8), 
                       ...
                       X6 = 0, 
                       X7 = 0)
pr.S.2020more <- generate_predictions(mS.2020, more.new, dhw.column)
