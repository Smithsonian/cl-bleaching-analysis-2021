...
# Reduced dataframe.
bleach.reduced.2017 <- bleach_2017[,c("bin.score",dhw.column,"region")]
...
pr.2017 <- generate_predictions(m.2017, newdata.2017$no.ev, dhw.column)
pr.2017$region <- factor(pr.2017$region, levels = c("North","Central","South"))

pr.S.2017 <- generate_predictions(mS.2017, newdata.2017$ev, dhw.column)
pr.S.2017$region <- factor(pr.S.2017$region, levels = c("North","Central","South"))

# Creating plot objects.
plot.2017 <- create_plot_object("2017", dhw.column, "DHW (°C-week)", " ", pr.2017)
plot.S.2017 <- create_plot_object("2017", dhw.column, "DHW (°C-week)", " ", pr.S.2017)
