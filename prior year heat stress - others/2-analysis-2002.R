...
variog.2002 <- create_variogram_object_RA("DHW_5km_max_2002", 
                                          bleach_2002, 
                                          residuals.2002)

variogS.2002 <- create_variogram_object_RA("DHW_5km_max_2002", 
                                           bleach_2002, 
                                           residuals.S.2002)

par(mfrow = c(2,1))
plot(variog.2002, main = "DHW Max 2002 - No SAC")
abline(h = variog.2002$var.mark)
plot(variogS.2002, main = "DHW Max 2002 - SAC")
...
# load("correlograms/correlograms2002.RData")
...
newdata.2002 <- generate_prediction_dataframe(dhw.column,
                                              bleach.reduced.2002, 
                                              nevs = ncol(eigenvectors.2002))

# Calculating predictions.
pr.2002 <- generate_predictions(dhw.column, m.2002, newdata.2002$no.ev)
pr.S.2002 <- generate_predictions(dhw.column, mS.2002, newdata.2002$ev)

# Creating plot objects.
plot.2002 <- create_plot_object(dhw.column, 
                                " ", 
                                "DHW in 1998",
                                " ",
                                pr.2002)
plot.S.2002 <- create_plot_object(dhw.column,
                                  " ", 
                                  "DHW in 1998",
                                  " ",
                                  pr.S.2002)
